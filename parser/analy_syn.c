...
void OPERATEUR_VERSION(){
    switch(symbols[cursor].token){
        case OPERATEUR_BINAIRE_XOR_TOKEN: test_sym(OPERATEUR_BINAIRE_XOR_TOKEN);break;
        case OPERATEUR_BINAIRE_NON_TOKEN: test_sym(OPERATEUR_BINAIRE_NON_TOKEN);break;
...
void DEFINITION_FONCTION(){
    ...
    || symbols[cursor].token==WHILE_TOKEN || symbols[cursor].token==READ_TOKEN || symbols[cursor].token==WRITE_TOKEN
    )
    {
       DECLARATION();
    }
...
void LISTE_PARAMETRE(){
    ...
    || symbols[cursor].token==TYPE_SIGNATURE_DONNEE_TOKEN || symbols[cursor].token==BYTES_TOKEN){
        ...
        while (symbols[cursor].token==VIRGULE_TOKEN &&(symbols[cursor+1].token==TYPE_ENTIER_TOKEN 
            || symbols[cursor+1].token==TYPE_BOOLEAN_TOKEN || symbols[cursor+1].token==TYPE_STRING_TOKEN 
            || symbols[cursor+1].token==TYPE_CLE_PUBLIQUE_TOKEN || symbols[cursor+1].token==TYPE_SIGNATURE_TOKEN 
            || symbols[cursor+1].token==TYPE_SIGNATURE_DONNEE_TOKEN || symbols[cursor+1].token==BYTES_TOKEN))
        {
...
void BLOC(){
    ...
    || symbols[cursor].token==TYPE_SIGNATURE_DONNEE_TOKEN || symbols[cursor].token==BYTES_TOKEN
    || symbols[cursor].token==IDENTIFIANT_TOKEN || symbols[cursor].token==REQUIRE_TIME_TOKEN 
    || symbols[cursor].token==REQUIRE_TOKEN || symbols[cursor].token==IF_TOKEN || symbols[cursor].token==WHILE_TOKEN
...
void LISTE_EXPRESSIONS(){
    ...
    || symbols[cursor].token==DATE_TOKEN || symbols[cursor].token==HEX_LITTERAL_TOKEN || symbols[cursor].token==OPERATEUR_NON_TOKEN  
    || symbols[cursor].token==MOINS_TOKEN || symbols[cursor].token==CROCHET_O_TOKEN
    || symbols[cursor].token==TX_VERSION_TOKEN || symbols[cursor].token==TX_HASHPREVOUTS_TOKEN || symbols[cursor].token==TX_HASHSEQUENCE_TOKEN
    || symbols[cursor].token==TX_OUTPOINT_TOKEN || symbols[cursor].token==TX_BYTECODE_TOKEN || symbols[cursor].token==TX_VALUE_TOKEN
    || symbols[cursor].token==TX_SEQUENCE_TOKEN || symbols[cursor].token==TX_HASHOUTPUTS_TOKEN || symbols[cursor].token==TX_LOCKTIME_TOKEN
    ...
    || symbols[cursor+1].token==TYPE_SIGNATURE_TOKEN || symbols[cursor+1].token==TYPE_SIGNATURE_DONNEE_TOKEN
    ...
    || symbols[cursor+1].token==DATE_TOKEN || symbols[cursor+1].token==HEX_LITTERAL_TOKEN || symbols[cursor+1].token==OPERATEUR_NON_TOKEN  
    || symbols[cursor+1].token==MOINS_TOKEN || symbols[cursor+1].token==CROCHET_O_TOKEN
    || symbols[cursor+1].token==TX_VERSION_TOKEN || symbols[cursor+1].token==TX_HASHPREVOUTS_TOKEN || symbols[cursor+1].token==TX_HASHSEQUENCE_TOKEN
    || symbols[cursor+1].token==TX_OUTPOINT_TOKEN || symbols[cursor+1].token==TX_BYTECODE_TOKEN || symbols[cursor+1].token==TX_VALUE_TOKEN
    || symbols[cursor+1].token==TX_SEQUENCE_TOKEN || symbols[cursor+1].token==TX_HASHOUTPUTS_TOKEN || symbols[cursor+1].token==TX_LOCKTIME_TOKEN
...
void EXPRESSION(){
    ...
    || symbols[cursor].token==TYPE_SIGNATURE_DONNEE_TOKEN || symbols[cursor].token==BYTES_TOKEN){
        ...
        test_sym(PARENTHESE_O_TOKEN);
        EXPRESSION();
        if(symbols[cursor].token==VIRGULE_TOKEN &&(symbols[cursor+1].token==PARENTHESE_O_TOKEN
    || symbols[cursor+1].token==TYPE_ENTIER_TOKEN || symbols[cursor+1].token==TYPE_BOOLEAN_TOKEN 
    || symbols[cursor+1].token==TYPE_STRING_TOKEN || symbols[cursor+1].token==TYPE_CLE_PUBLIQUE_TOKEN 
    || symbols[cursor+1].token==TYPE_SIGNATURE_TOKEN || symbols[cursor+1].token==TYPE_SIGNATURE_DONNEE_TOKEN
    || symbols[cursor+1].token==BYTES_TOKEN || symbols[cursor+1].token==NOUVEAU_TOKEN
    ...
    || symbols[cursor+1].token==TX_HASHTYPE_TOKEN || symbols[cursor+1].token==TX_PREIMAGE_TOKEN
        ...
        case INVERSER_TOKEN: test_sym(INVERSER_TOKEN); break;
        case TAILLE_TOKEN: test_sym(TAILLE_TOKEN); break;
        ...
        case DIV_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_MODULO_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        ...
        case OPERATEUR_DIFFERENT_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_BINAIRE_ET_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_BINAIRE_XOR_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_BINAIRE_OU_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_ET_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        case OPERATEUR_OU_TOKEN: DEUXIEME_EXPRESSION_BINAIRE(); break;
        }
    }
    else if(symbols[cursor].token==TRUE_TOKEN || symbols[cursor].token==FALSE_TOKEN || symbols[cursor].token==NOMBRE_LITTERAL_TOKEN
    || symbols[cursor].token==GUILLEMET_SIMPLE_TOKEN || symbols[cursor].token==GUILLEMET_TOKEN || symbols[cursor].token==DATE_TOKEN
    || symbols[cursor].token==HEX_LITTERAL_TOKEN
        ...
        case SPLIT_TOKEN: {test_sym(SPLIT_TOKEN);
                            test_sym(PARENTHESE_O_TOKEN);
                            EXPRESSION();
                            test_sym(PARENTHESE_F_TOKEN);
                            break;
...
void DEUXIEME_EXPRESSION_BINAIRE(){
    ...
    case DIV_TOKEN: test_sym(DIV_TOKEN); break;
    case OPERATEUR_MODULO_TOKEN: test_sym(OPERATEUR_MODULO_TOKEN); break;
    case PLUS_TOKEN: test_sym(PLUS_TOKEN); break;
    case MOINS_TOKEN: test_sym(MOINS_TOKEN); break;
    case INF_TOKEN: test_sym(INF_TOKEN); break;
    case INFEG_TOKEN: test_sym(INFEG_TOKEN); break;
    case SUP_TOKEN: test_sym(SUP_TOKEN); break;
    case SUPEG_TOKEN: test_sym(SUPEG_TOKEN); break;
    case EG_TOKEN: test_sym(EG_TOKEN); break;
    case OPERATEUR_DIFFERENT_TOKEN: test_sym(OPERATEUR_DIFFERENT_TOKEN); break;
    case OPERATEUR_BINAIRE_ET_TOKEN: test_sym(OPERATEUR_BINAIRE_ET_TOKEN); break;
    case OPERATEUR_BINAIRE_XOR_TOKEN: test_sym(OPERATEUR_BINAIRE_XOR_TOKEN); break;
    case OPERATEUR_BINAIRE_OU_TOKEN: test_sym(OPERATEUR_BINAIRE_OU_TOKEN); break;
    case OPERATEUR_ET_TOKEN: test_sym(OPERATEUR_ET_TOKEN); break;
    case OPERATEUR_OU_TOKEN: test_sym(OPERATEUR_OU_TOKEN); break;
...
void LITTERAL(){
    switch (symbols[cursor].token)
    {
    case TRUE_TOKEN : BOOLEAN_LITTERAL_TOKEN(); break;
    case FALSE_TOKEN : BOOLEAN_LITTERAL_TOKEN(); break;
    case NOMBRE_LITTERAL_TOKEN : NOMBRE_LITTERAL(); break;
    case GUILLEMET_SIMPLE_TOKEN : STRING_LITTERAL_TOKEN(); break;
    case GUILLEMET_TOKEN : STRING_LITTERAL_TOKEN(); break;
...
void NOM_TYPE(){
    switch (symbols[cursor].token)
    {
    case TYPE_ENTIER_TOKEN: test_sym(TYPE_ENTIER_TOKEN); break;
    case TYPE_BOOLEAN_TOKEN: test_sym(TYPE_BOOLEAN_TOKEN); break;
    case TYPE_STRING_TOKEN: test_sym(TYPE_STRING_TOKEN); break;
    case TYPE_CLE_PUBLIQUE_TOKEN: test_sym(TYPE_CLE_PUBLIQUE_TOKEN); break;
    case TYPE_SIGNATURE_TOKEN: test_sym(TYPE_SIGNATURE_TOKEN); break;
    case TYPE_SIGNATURE_DONNEE_TOKEN: test_sym(TYPE_SIGNATURE_DONNEE_TOKEN); break;
...
void UNITE_TOKEN(){
    switch (symbols[cursor].token)
    {
    case SATOSHIS_TOKEN: test_sym(SATOSHIS_TOKEN); break;
    case SATS_TOKEN: test_sym(SATS_TOKEN); break;
    case FINNEY_TOKEN: test_sym(FINNEY_TOKEN); break;
    case BITS_TOKEN: test_sym(BITS_TOKEN); break;
    case BITCOIN_TOKEN: test_sym(BITCOIN_TOKEN); break;
    case SECONDS_TOKEN: test_sym(SECONDS_TOKEN); break;
    case MINUTES_TOKEN: test_sym(MINUTES_TOKEN); break;
    case HOURS_TOKEN: test_sym(HOURS_TOKEN); break;
    case DAYS_TOKEN: test_sym(DAYS_TOKEN); break;
...
void STRING_LITTERAL_TOKEN(){
    switch (symbols[cursor].token)
    {
    case GUILLEMET_SIMPLE_TOKEN:{ test_sym(GUILLEMET_SIMPLE_TOKEN);
...
void CHAMP_AVANT_IMAGE_TOKEN(){
    switch (symbols[cursor].token)
    {
    case TX_VERSION_TOKEN: test_sym(TX_VERSION_TOKEN); break;
    case TX_HASHPREVOUTS_TOKEN: test_sym(TX_HASHPREVOUTS_TOKEN); break;
    case TX_HASHSEQUENCE_TOKEN: test_sym(TX_HASHSEQUENCE_TOKEN); break;
    case TX_OUTPOINT_TOKEN: test_sym(TX_OUTPOINT_TOKEN); break;
    case TX_BYTECODE_TOKEN: test_sym(TX_BYTECODE_TOKEN); break;
    case TX_VALUE_TOKEN: test_sym(TX_VALUE_TOKEN); break;
    case TX_SEQUENCE_TOKEN: test_sym(TX_SEQUENCE_TOKEN); break;
    case TX_HASHOUTPUTS_TOKEN: test_sym(TX_HASHOUTPUTS_TOKEN); break;
    case TX_LOCKTIME_TOKEN: test_sym(TX_LOCKTIME_TOKEN); break;
    case TX_HASHTYPE_TOKEN: test_sym(TX_HASHTYPE_TOKEN); break;
